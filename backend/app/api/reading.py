...
class ConvertRequest(BaseModel):
    text: str = Field(..., min_length=1, description="原始中文文本")
    level: int = Field(default=1, ge=1, le=3, description="替换等级 1-3")
...
# API 英文替换 Prompt
# ============================================================
def build_prompt(text: str, level: int) -> str:
    level_config = {
    ...
    return f"""你是一个中英混合文本生成器，用于帮助中文母语者渐进式学习英语。
...
async def call_deepseek(text: str, level: int) -> tuple[str, list[dict]]:
...
async def convert_text(text: str, level: int = 1) -> dict:
    ...
    if DEEPSEEK_API_KEY:
        try:
            result, vocabulary = await call_deepseek(text, level)
            return {
                "result": result,
...
async def get_articles(difficulty: int | None = Query(default=None, ge=1, le=3)):
    """获取文章列表，可按难度筛选"""
    if difficulty is not None:
        return [a for a in ARTICLES if a["difficulty"] == difficulty]
    return ARTICLES
...
async def convert(req: ConvertRequest):
    """将中文文本转换为中英混合文本"""
    result = await convert_text(req.text, req.level)
    return result
